...
const db = require('../config/database');
const { checkAdmin, checkDoctor } = require("../middleware/auth");
const bcrypt = require("bcrypt"); // Mova o require do bcrypt para o topo, seguindo as melhores práticas

const patientController = {
// ... (Métodos getProfile e getStats permanecem inalterados) ...
      getProfile: async (req, res) => {
    try {
      const userId = req.user.id;
  ...
  getStats: async (req, res) => {
    ...
    getAllPatients: async (req, res) => {
        try {
            // Verifica permissão (Admin ou Médico)
            if (!checkDoctor(req.user) && !checkAdmin(req.user)) {
                return res.status(403).json({ error: "Acesso negado" });
            }
            ...
            // Se for médico, filtra apenas pacientes do médico (CORREÇÃO DE LÓGICA: busca o ID do médico na tabela 'medicos')
            if (checkDoctor(req.user)) {
                // 1. Encontra o ID do registro médico (medicos.id) usando o ID do usuário (usuarios.id)
                const medicoResult = await db.query(
                    'SELECT id FROM medicos WHERE usuario_id = $1',
                    [req.user.id] // req.user.id é o usuarios.id (do médico)
                );

                if (medicoResult.rows.length === 0) {
                    // Deveria ser impossível, mas protege contra inconsistências
                    return res.status(404).json({ error: "Registro médico não encontrado" });
                }

                const medicoId = medicoResult.rows[0].id; // Este é o ID correto para a FK 'medico_responsavel_id'

                // 2. Filtra a query usando o ID do médico
    ...
    getPatientById: async (req, res) => {
        try {
            ...
            // A query deve unir usuários e pacientes para obter o usuario_id do paciente
            const patientResult = await db.query(
            ...
            // O medico_id no req.user é o usuarios.id, o patient.medico_responsavel_id é o medicos.id.
            // Precisamos encontrar o medicos.id do médico logado para a checagem.
            ...
            // ... (Continua com medicacoes e reminders)
    ...
    // ... (Método createPatient permanece inalterado, usa checkDoctor/checkAdmin) ...

    // ... (Método updatePatient - Necessita do mesmo ajuste de getPatientById)
    updatePatient: async (req, res) => {
        try {
            const patientId = req.params.id;
    ...
    // ... (Os demais métodos getMedicalData, updateMedicalData, getDialysisHistory, assignDoctor permanecem inalterados, mas requerem a checagem de permissão interna se usarem medico_responsavel_id)

    // Ajuste em assignDoctor (Não precisa de alteração de ID, mas corrigindo a coluna)
    assignDoctor: async (req, res) => {
        try {
            if (!checkAdmin(req.user)) {
                return res.status(403).json({ error: "Acesso negado" });
